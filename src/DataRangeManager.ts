...
export const setupDataRangeCells = () => {
  ...
  sheet
    .getRange(CHART_CONFIG.EARNINGS.LABEL_CELL)
    .setValue(CHART_CONFIG.EARNINGS.LABEL)
  sheet
    .getRange(CHART_CONFIG.EARNINGS.DATA_RANGE_CELL)
    .setValue(CHART_CONFIG.EARNINGS.DEFAULT_DATA_RANGE)

  sheet
    .getRange(CHART_CONFIG.EXPENSES.LABEL_CELL)
    .setValue(CHART_CONFIG.EXPENSES.LABEL)
  sheet
    .getRange(CHART_CONFIG.EXPENSES.DATA_RANGE_CELL)
    .setValue(CHART_CONFIG.EXPENSES.DEFAULT_DATA_RANGE)

  sheet
    .getRange(CHART_CONFIG.INVESTMENT_PLANS.LABEL_CELL)
    .setValue(CHART_CONFIG.INVESTMENT_PLANS.LABEL)
  sheet
  ...
  sheet
    .getRange(SOURCE_RANGE_CONFIG.EARNINGS.LABEL_CELL)
    .setValue(SOURCE_RANGE_CONFIG.EARNINGS.LABEL)
  sheet
    .getRange(SOURCE_RANGE_CONFIG.EARNINGS.VALUE_CELL)
    .setValue(SOURCE_RANGE_CONFIG.EARNINGS.DEFAULT_RANGE)
  sheet
    .getRange(SOURCE_RANGE_CONFIG.FIXED_EXPENSES.LABEL_CELL)
    .setValue(SOURCE_RANGE_CONFIG.FIXED_EXPENSES.LABEL)
  sheet
    .getRange(SOURCE_RANGE_CONFIG.FIXED_EXPENSES.VALUE_CELL)
    .setValue(SOURCE_RANGE_CONFIG.FIXED_EXPENSES.DEFAULT_RANGE)
  sheet
    .getRange(SOURCE_RANGE_CONFIG.VARIABLE_EXPENSES.LABEL_CELL)
    .setValue(SOURCE_RANGE_CONFIG.VARIABLE_EXPENSES.LABEL)
  sheet
    ...
    if (ranges.length > 0) {
      ranges.forEach(range => {
        ...
        const earningsDataRange = sheet
          ...
          .getValue()
        if (
          rangeNotation.includes(earningsDataRange) &&
          !foundCharts.has('earnings')
        ) {
        ...
        const expenseDataRange = sheet
          ...
          .getValue()
        if (
          rangeNotation.includes(expenseDataRange) &&
          !foundCharts.has('expense')
        ) {
        ...
        if (
          ...
          log(
            `Found Investment Plans Chart using range: ${CHART_CONFIG.INVESTMENT_PLANS.DEFAULT_DATA_RANGE}`
          )
...
export const resetEarningsDataRange = () => {
  ...
  log(
    `Reset earnings data range to default: ${CHART_CONFIG.EARNINGS.DEFAULT_DATA_RANGE}`
  )
}
...
export const resetExpenseDataRange = () => {
  ...
  log(
    `Reset expense data range to default: ${CHART_CONFIG.EXPENSES.DEFAULT_DATA_RANGE}`
  )
}

export const resetInvestmentPlanDataRange = () => {
  updateInvestmentPlanDataRange(
    CHART_CONFIG.INVESTMENT_PLANS.DEFAULT_DATA_RANGE
  )
  log(
    `Reset investment plan data range to default: ${CHART_CONFIG.INVESTMENT_PLANS.DEFAULT_DATA_RANGE}`
  )
}
